...
impl TokenGenerator for MockTokenGeneratorintro {
    ...
    fn validate_token(&self, token: &str, _aud: Option<&str>, _sub: &str, _required_scope: &str) -> Result<TokenData<Claims>, TokenError> {
        let is_expired = self.expired_tokens.lock().unwrap().contains(&token.to_string());
        ...
        println!("Current timestamp {}, Claims exp: {}", current_timestamp, claims.exp);
...
async fn test_expired_token() {
    ...
    assert!(response.is_ok(), "Expected Ok response for expired token introspection");
    let introspection_response = response.unwrap();
...
async fn test_valid_client_credentials() {
    ...
    let token = "valid_access_token";
    let client_credentials = Some(("valid_client_id".to_string(), "valid_client_secret".to_string()));
...
async fn test_invalid_client_credentials() {
    ...
    let token = "valid_access_token";
    let client_credentials = Some(("invalid_client_id".to_string(), "invalid_client_secret".to_string()));
