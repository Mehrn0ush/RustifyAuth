...
pub async fn revoke_token_endpoint(
    ...
    let mut token_store = token_store.lock().map_err(|_| warp::reject::custom(TokenError::InternalError))?;
    ...
    fn into_response(self) -> warp::reply::Response {
        match self {
            TokenError::InvalidToken => {
                warp::reply::with_status("Invalid or expired token", StatusCode::BAD_REQUEST).into_response()
            }
            TokenError::InvalidTokenTypeHint => {
                warp::reply::with_status("Invalid token_type_hint", StatusCode::BAD_REQUEST).into_response()
            }
            TokenError::InternalError => {
                warp::reply::with_status("Internal server error", StatusCode::INTERNAL_SERVER_ERROR).into_response()
            }
            ...
            _ => warp::reply::with_status("Unhandled error", StatusCode::BAD_REQUEST).into_response(),
        }
    }
}
