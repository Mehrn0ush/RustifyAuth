...
use crate::core::types::{TokenResponse, TokenError};
...
impl AuthorizationCodeFlow {
    ...
    pub fn validate_pkce_challenge(stored_challenge: &str, verifier: &str) -> Result<(), PkceError> {
        ...
        if constant_time_eq::constant_time_eq(stored_challenge.as_bytes(), generated_challenge.as_bytes()) {
            Ok(())
        } else {
    ...
    pub fn generate_authorization_code(
        ...
        let pkce_challenge = generate_pkce_challenge(pkce_verifier).map_err(|_| AuthorizationError::InvalidPKCE)?;
    ...
    pub fn exchange_code_for_token(
        ...
        eprintln!("Attempting to retrieve the authorization code for: {}", code);
        ...
        eprintln!("Authorization code retrieved successfully: {:?}", stored_code);
...
/******************* 
        ...
        *********************/
        ...
        let access_token = self
            .token_generator
            .generate_access_token(&stored_code.client_id, &stored_code.code, &stored_code.scope)?;
        

        let refresh_token = self
            .token_generator
            .generate_refresh_token(&stored_code.client_id, &stored_code.code, &stored_code.scope)?;
...
impl TokenGenerator for MockTokenGeneratorWithExpiry {
    ...
    fn validate_token(
        ...
        let creation_time = self.token_creation_times
            .lock()
            .unwrap()
            .get(token)
            .copied();
...
mod tests {
    ...
    use crate::core::token::{JwtTokenGenerator, InMemoryTokenStore};
    ...
    fn test_generate_authorization_code() {
        ...
        let auth_code = auth_code_flow.generate_authorization_code(
            "client_id",
            "redirect_uri",
            &pkce_verifier,
            scope,
    ...
    fn test_exchange_valid_code_for_token() {
        ...
        let token_response = auth_code_flow.exchange_code_for_token(
            &auth_code.code,
            &pkce_verifier,
        );
        ...
        assert!(token_response.is_ok(), "Expected Ok response, but got an error.");
    ...
    fn test_exchange_expired_code() {
        ...
        assert!(token_response.is_err(), "Expected an error response for expired code.");
        assert_eq!(
...
fn test_expired_token_generation() {
    ...
    assert!(validation_result.is_err(), "Expected token validation to fail due to expiration.");
...
fn test_reuse_authorization_code() {
    ...
    let first_response = auth_code_flow.exchange_code_for_token(
        &auth_code.code,
        &pkce_verifier,
    );
   // eprintln!("Revoke result: {:?}", code_revoke_result);
    ...
    let second_response = auth_code_flow.exchange_code_for_token(
        &auth_code.code,
        &pkce_verifier,
    );
    assert!(second_response.is_err(), "Second exchange should fail due to resused authorizzation code");
...
fn test_malformed_pkce_verifier() {
    ...
    assert!(result.is_err(), "Expected error due to invalid PKCE verifier.");
...
fn test_revoked_tokens() {
    ...
    assert!(auth_code_flow.code_store.is_code_revoked(&auth_code.code), "Code should be revoked.");

    // Step 4: Attempt to exchange the revoked code, which should fail
    let second_token_response = auth_code_flow.exchange_code_for_token(
        &auth_code.code,
        &pkce_verifier,
    );
    assert!(second_token_response.is_err(), "Expected error when exchanging a revoked code.");
